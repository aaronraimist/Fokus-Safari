...
chrome.storage.sync.get(
	...
	function( options ) {
		...
		(function(){
			...
			var PADDING = options['fokusPadding'] ? parseFloat( options['fokusPadding'] ) : 5;

			// Opacity of the overlay
			var OPACITY = options['fokusOpacity'] ? parseFloat( options['fokusOpacity'] ) : 0.75;

			// Key modifier that needs to be held down for overlay to appear
			var MODIFIER = options['fokusModifier'];
            
            // Background color for selected text
            var BACKGROUND = options['fokusHighlightColor'];
